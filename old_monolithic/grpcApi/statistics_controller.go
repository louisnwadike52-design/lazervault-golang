...
type StatisticsController struct {
	...
	wrappedService *services.WrappedService
	userService    services.IUserService
}

func NewStatisticsController(db *gorm.DB, userService services.IUserService, aiService *services.AIStatisticsService, wrappedService *services.WrappedService) *StatisticsController {
...
func (c *StatisticsController) GetFinancialWrapped(ctx context.Context, req *pb.GetFinancialWrappedRequest) (*pb.GetFinancialWrappedResponse, error) {
	user, accessToken, err := getUserAndTokenFromContext(ctx, c.userService)
	if err != nil {
		return nil, err
	}

	response, err := c.wrappedService.GetFinancialWrapped(ctx, user.ID, accessToken, req)
	if err != nil {
		return nil, status.Error(codes.Internal, err.Error())
	}
