...
func (s *TransferService) InitiateTransfer(ctx context.Context, fromUserID uint, req TransferRequest) (*TransferResponse, error) {
	...
	if transfer.Status == models.TransferStatusProcessing {
		// CRITICAL FIX: Lock account row BEFORE debiting to prevent race conditions
		// This prevents concurrent transfers from debiting more than available balance
		var account models.Account
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).
			Where("id = ?", transfer.FromAccountID).
			First(&account).Error; err != nil {
			tx.Rollback()
			return nil, fmt.Errorf("failed to lock source account %d for transfer %d: %w", transfer.FromAccountID, transfer.ID, err)
		}

		// Check balance again AFTER lock (in case balance changed between initial check and now)
		if account.Balance < transfer.TotalAmount {
			tx.Rollback()
			return nil, fmt.Errorf("insufficient balance in account %d after lock: have %d, need %d",
				transfer.FromAccountID, account.Balance, transfer.TotalAmount)
		}

		// Debit the total amount directly from the source account (still holding lock)
